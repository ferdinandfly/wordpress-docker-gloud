...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_instances.php';
        require_once get_template_directory() . '/inc/ReduxCore/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_functions.php';
        require_once get_template_directory() . '/inc/ReduxCore/inc/class.p.php';

        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_filesystem.php';

        require_once get_template_directory() . '/inc/ReduxCore/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        //require_once get_template_directory() . '/inc/ReduxCore/inc/themecheck/class.redux_themecheck.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( get_template_directory() . '/inc/ReduxCore' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'zap' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'zap' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'zap' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'zap' );
                }
...
// __construct()
            ...
            private function _internationalization() {
                 ...
                 * @param string     'zap'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'zap' );
                ...
                load_textdomain( 'zap', self::$_dir . 'languages/' . $locale . '.mo' );
            }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( get_template_directory() . '/inc/ReduxCore/inc/tracking.php' ) ) {
                    require_once get_template_directory() . '/inc/ReduxCore/inc/tracking.php';
...
// get_wordpress_data()
            ...
            public function show( $opt_name, $default = '' ) {
                ...
                if ( ! is_array( $option ) && $option != '' ) {
                    ...
                    echo do_shortcode($this->_get_default( $opt_name, $default ));
                }
            }
// show()
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    add_theme_page( $this->args['page_title'], $this->args['menu_title'], $this->args['page_permissions'], $this->args['page_slug'], array( $this, 'generate_panel') );
                } else {
            ...
            public function _admin_bar_menu() {
                ...
                if ( $menu ) {
                    ...
                    $nodeargs = array(
                        ...
                        'title' => $title,
                        // $theme_data->get( 'Name' ) . " " . esc_html__( 'Options', 'zap' ),
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        <script>
                            ...
                            WebFontConfig['google'] = {families: [<?php echo do_shortcode($typography->makeGoogleWebfontString ( $this->typography )) ?>]};
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'zap' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'zap' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'zap' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'zap' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'zap' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'zap' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'zap' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/inc/ReduxCore/inc/extensions/';
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => esc_html__( 'Invalid security credential, please reload the page and try again.', 'zap' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'zap' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                        ...
                        if ( is_array( $parentValue ) ) {
                            foreach ( $parentValue as $idx => $val ) {
                                if ( is_array( $checkValue ) ) {
                                    ...
                                    if ( $val == $checkValue ) {
                                        echo do_shortcode($val) . '<br>';
                                        echo do_shortcode($checkValue);
                                        $return = true;
                                    }
                                }
                            }
