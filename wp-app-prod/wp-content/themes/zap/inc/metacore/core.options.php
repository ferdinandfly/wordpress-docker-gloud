    ...
    if (is_admin() && !empty($params['id']) && isset($core_options)) {
        ...
        elseif ($pagenow=='post-new.php' || $pagenow=='post.php'){
            ...
            */
            ...
            // Get value
            if(isset($zo_meta->{$params['id']})){  //php 7.0
                $params['value'] = $zo_meta->{$params['id']};  //php 7.0
            } else {
                $params['value'] = null;
            }
...
class ZoCoreControl
{
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php esc_html_e('Replace Image', 'zap'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php esc_html_e('Remove Image', 'zap'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php esc_html_e('Add Image', 'zap'); ?>"></i></li>
    ...
    private function social($params){
		...
		<table class="wp-list-table widefat fixed pages zo_social_team">
			<thead>
				<tr>
					<th><?php esc_html_e('Title', 'zap');?></th>
					<th><?php esc_html_e('Link', 'zap');?></th>
					<th><?php esc_html_e('Icon class', 'zap');?></th>
					<th><?php esc_html_e('Actions', 'zap');?></th>
				</tr>
			</thead>
			<tfoot>
				<tr>
				...
				<tr>
					<td colspan="3">
						<a class="button-primary" id="btn_add_social" href="javascript:void(0);"><?php esc_html_e('Add new', 'zap');?></a>
					</td>
				</tr>
			</tfoot>
			<tbody  id="the-list">
				<?php
					if(count($zo_social_team)):
						...
						<tr>
							...
							<td><a title="Click to edit this item" href="javascript:void(0)" class="zo_social"><?php esc_html_e('Edit', 'zap')?></a>|<a class="zo_social_remove" href="javascript:void(0)"><?php esc_html_e('Remove', 'zap')?></a></td>
						</tr>
						<?php endforeach;
					else: ?>
						<tr>
							<td class="zo_no_item" colspan="3"><?php esc_html_e('No item!', 'zap');?></td>
						</tr>
					<?php endif;
				?>
			</tbody>
