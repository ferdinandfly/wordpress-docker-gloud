...
class WC_Widget_Cart_Search extends WP_Widget {
    public function __construct() {
        parent::__construct(
            ...
            esc_html__('Cart & Search', 'zap'), // Name
            array('description' => esc_html__("Display the user's Cart and Search form in the sidebar.", 'zap'),) // Args
        );
                        ...
                        <div class="shopping_cart_dropdown_inner">
                            ...
                            <ul class="cart_list product_list_widget">
                                ...
                                <?php else : ?>
                                    ...
                                    <li class="cart-list clearfix empty"><?php esc_html_e( 'No products in the cart.', 'zap' ); ?></li>
                            ...
                            <?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
                                <div class="cart-total">
                                    <span class="total"><?php esc_html_e( 'Sub Total', 'zap' ); ?>:<span><?php echo do_shortcode($woocommerce->cart->get_cart_subtotal()); ?></span></span>
                                    <a href="<?php echo esc_url($woocommerce->cart->get_checkout_url()); ?>" class="btn btn-primary btn-checkout"><?php esc_html_e( 'Process To Checkout', 'zap'); ?></a>
                                    <a href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" class="btn btn-cart"><?php esc_html_e( 'View Shopping Cart', 'zap'); ?></a>
                                </div>
                            <?php endif; ?>
                        </div>
                ...
                <?php if($show_search):?>
                    <div class="widget_searchform_content">
                        <form method="get" action="<?php echo esc_url( home_url( '/'  ) );?>">
                            ...
                            <input type="submit" value="<?php echo esc_html__( 'Search', 'zap' )?>" />
                            <?php if($woocommerce):?>
    ...
    function form( $instance ) {
		...
		<p>
            <label for="<?php echo esc_url($this->get_field_id('show_cart')); ?>"><?php esc_html_e( 'Show Cart:', 'zap' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_cart')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_cart')); ?>">
				<option value="0" <?php selected($show_cart,0); ?>><?php echo esc_html__('No','zap'); ?></option>
				<option value="1" <?php selected($show_cart,1); ?>><?php echo esc_html__('Yes','zap'); ?></option>
			</select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('show_search')); ?>"><?php esc_html_e( 'Show Search:', 'zap' ); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('show_search')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_search')); ?>">
				<option value="0" <?php selected($show_search,0); ?>><?php echo esc_html__('No','zap'); ?></option>
				<option value="1" <?php selected($show_search,1); ?>><?php echo esc_html__('Yes','zap'); ?></option>
			</select>
        </p>
    <?php
