...
class Zo_Flickr_Badges_Widget extends WP_Widget {
	...
	**/			
	function __construct() {
		...
		$widget_options = array('classname' => 'widget_flickr', 'description' => esc_html__( 'Displays a Flickr photo stream from an ID', 'zap' ) );
		
		// Create the widget
        parent::__construct($this->prefix, esc_html__('ZO Flickr Badge', 'zap'), $widget_options, $control_options );
	...
	**/		
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['flickr_id'] ) )
			...
			echo '<p>' . esc_html__('Please provide an Flickr ID', 'zap') . '</p>';
	...
	**/		
	function form( $instance ) {
		...
		$defaults = array(
			'title'			=> esc_html__( 'Flickr Widget', 'zap' ),
			'type'			=> 'user',
		...
		$types = array( 
			'user'  => esc_html__( 'user', 'zap' ),
			'group' => esc_html__( 'group', 'zap' )
		);
		$sizes = array(
			's' => esc_html__( 'Standard', 'zap' ),
			't' => esc_html__( 'Thumbnail', 'zap' ),
			'm' => esc_html__( 'Medium', 'zap' )
		);
		$displays = array( 
			'latest' => esc_html__( 'latest', 'zap' ),
			'random' => esc_html__( 'random', 'zap' )
		);
		
		$tabs = array( 
			esc_html__( 'General', 'zap' ),
			esc_html__( 'Customs', 'zap' ),
		);
		...
		<div id="fbw-<?php echo esc_attr($this->id) ; ?>" class="totalControls tabbable tabs-left">
			...
			<ul class="tab-content">
				<li class="tab-pane <?php if ( $instance['tab'][0] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title', 'zap'); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'Give the widget title, or leave it empty for no title.', 'zap' ); ?></span>
							<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php esc_html_e( 'Type', 'zap' ); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'The type of images from user or group.', 'zap' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID', 'zap'); ?></label>
							...
							<span class="controlDesc"><?php printf( __( 'Put the flickr ID here, go to <a href="%s" target="_blank">Flickr NSID Lookup</a> if you don\'t know your ID. Example: 71865026@N00', 'zap' ), esc_url('http://goo.gl/PM6rZ')); ?></span>
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number', 'zap'); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'Number of images shown from 1 to 10', 'zap' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('display')); ?>"><?php esc_html_e('Display Method', 'zap'); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'Get the image from recent or use random function.', 'zap' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('sizes')); ?>"><?php esc_html_e( 'Sizes', 'zap' ); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'Represents the size of the image', 'zap' ); ?></span>
				...
				<li class="tab-pane <?php if ( $instance['tab'][1] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('intro_text')); ?>"><?php esc_html_e( 'Intro Text', 'zap' ); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'This option will display addtional text before the widget content and HTML supports.', 'zap' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('outro_text')); ?>"><?php esc_html_e( 'Outro Text', 'zap' ); ?></label>
							<span class="controlDesc"><?php esc_html_e( 'This option will display addtional text after widget and HTML supports.', 'zap' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('custom')); ?>"><?php esc_html_e( 'Custom Script & Stylesheet', 'zap' ) ; ?></label>
							<span class="controlDesc"><?php esc_html_e( 'Use this box for additional widget CSS style of custom javascript. Current widget selector: ', 'zap' ); ?><?php echo '<tt>#' . $this->id . '</tt>'; ?></span>
