...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			// Load class strings.
			$this->strings = array(
				'page_title'                      => esc_html__( 'Install Required Plugins', 'zap' ),
				'menu_title'                      => esc_html__( 'Install Plugins', 'zap' ),
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'zap' ),
				'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'zap' ),
				...
				'return'                          => esc_html__( 'Return to Required Plugins Installer', 'zap' ),
				'dashboard'                       => esc_html__( 'Return to the dashboard', 'zap' ),
				'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'zap' ),
				'activated_successfully'          => esc_html__( 'The following plugin was activated successfully:', 'zap' ),
				'plugin_already_active'           => esc_html__( 'No action taken. Plugin %1$s was already active.', 'zap' ),
				'plugin_needs_higher_version'     => esc_html__( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'zap' ),
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'zap' ),
				'dismiss'                         => esc_html__( 'Dismiss this notice', 'zap' ),
				'contact_admin'                   => esc_html__( 'Please contact the administrator of this site for help.', 'zap' ),
			);
		...
		public function filter_plugin_action_links_update( $actions ) {
			$actions['update'] = sprintf(
				...
				esc_html__( 'This plugin needs to be updated to be compatible with your theme.', 'zap' ),
		...
		protected function add_admin_menu( array $args ) {
			if ( has_filter( 'tgmpa_admin_menu_use_add_theme_page' ) ) {
				_deprecated_function( 'The "tgmpa_admin_menu_use_add_theme_page" filter', '2.5.0', esc_html__( 'Set the parent_slug config variable instead.', 'zap' ) );
			}
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				if ( ! empty( $this->strings['nag_type'] ) ) {
					add_settings_error( 'zap', 'zap', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
				} else {
					$nag_class = version_compare( $this->wp_version, '3.8', '<' ) ? 'updated' : 'update-nag';
					add_settings_error( 'zap', 'zap', $rendered, $nag_class );
				}
			}
	...
	if ( ! function_exists( 'load_tgm_plugin_activation' ) ) {
		...
		function load_tgm_plugin_activation() {
			$GLOBALS['zap'] = TGM_Plugin_Activation::get_instance();
		}
	}
...
if ( ! function_exists( 'zap' ) ) {
	...
	function tgmpa( $plugins, $config = array() ) {
		$instance = call_user_func( array( get_class( $GLOBALS['zap'] ), 'get_instance' ) );
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		 ...
		 */
		public function __construct() {
			$this->tgmpa = call_user_func( array( get_class( $GLOBALS['zap'] ), 'get_instance' ) );
		...
		protected function get_plugin_advise_type_text( $required ) {
			if ( true === $required ) {
				return esc_html__( 'Required', 'zap' );
			}

			return esc_html__( 'Recommended', 'zap' );
		}
		...
		protected function get_plugin_source_type_text( $type ) {
			...
			switch ( $type ) {
				case 'repo':
					$string = esc_html__( 'WordPress Repository', 'zap' );
					break;
				case 'external':
					$string = esc_html__( 'External Source', 'zap' );
					break;
				case 'bundled':
					$string = esc_html__( 'Pre-Packaged', 'zap' );
					break;
			}
		...
		protected function get_plugin_status_text( $slug ) {
			if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
				return esc_html__( 'Not Installed', 'zap' );
			}

			if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
				$install_status = esc_html__( 'Installed But Not Activated', 'zap' );
			} else {
				$install_status = esc_html__( 'Active', 'zap' );
			}

			$update_status = '';

			if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Required Update not Available', 'zap' );

			} elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
				$update_status = esc_html__( 'Requires Update', 'zap' );

			} elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
				$update_status = esc_html__( 'Update recommended', 'zap' );
			}
		...
		public function column_version( $item ) {
			...
			if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Installed version:', 'zap' ) . '</p>',
					$color,
					$installed
				);
			}
			...
			if ( ! empty( $item['minimum_version'] ) ) {
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' . esc_html__( 'Minimum required version:', 'zap' ) . '</p>',
			...
			if ( ! empty( $item['available_version'] ) ) {
				...
				$output[] = sprintf(
					'<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' . esc_html__( 'Available version:', 'zap' ) . '</p>',
					$color,
		...
		public function no_items() {
			printf( wp_kses_post( esc_html__( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', 'zap' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'plugin' => esc_html__( 'Plugin', 'zap' ),
				'source' => esc_html__( 'Source', 'zap' ),
				'type'   => esc_html__( 'Type', 'zap' ),
			);

			if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
				$columns['version'] = esc_html__( 'Version', 'zap' );
				$columns['status']  = esc_html__( 'Status', 'zap' );
			}
		...
		public function get_bulk_actions() {
			...
			if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
				if ( current_user_can( 'install_plugins' ) ) {
					$actions['tgmpa-bulk-install'] = esc_html__( 'Install', 'zap' );
				}
			}

			if ( 'install' !== $this->view_context ) {
				if ( current_user_can( 'update_plugins' ) ) {
					$actions['tgmpa-bulk-update'] = esc_html__( 'Update', 'zap' );
				}
				if ( current_user_can( 'activate_plugins' ) ) {
					$actions['tgmpa-bulk-activate'] = esc_html__( 'Activate', 'zap' );
				}
			}
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
				...
				if ( empty( $_POST['plugin'] ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins were selected to be installed. No action taken.', 'zap' );
					} else {
						$message = esc_html__( 'No plugins were selected to be updated. No action taken.', 'zap' );
					}
				...
				if ( empty( $plugins_to_install ) ) {
					if ( 'install' === $install_type ) {
						$message = esc_html__( 'No plugins are available to be installed at this time.', 'zap' );
					} else {
						$message = esc_html__( 'No plugins are available to be updated at this time.', 'zap' );
					}
...
if ( ! function_exists( 'tgmpa_load_bulk_installer' ) ) {
	...
	function tgmpa_load_bulk_installer() {
		...
		if ( ! isset( $GLOBALS['zap'] ) ) {
			return;
		}

		// Get TGMPA class instance.
		$tgmpa_instance = call_user_func( array( get_class( $GLOBALS['zap'] ), 'get_instance' ) );
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'zap' );
						$this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'zap' );
					}
				...
				class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						if ( 'update' === $this->options['install_type'] ) {
							parent::add_strings();
							$this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Updating Plugin %1$s (%2$d/%3$d)', 'zap' );
						} else {
							$this->upgrader->strings['skin_update_failed_error'] = esc_html__( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'zap' );
							$this->upgrader->strings['skin_update_failed']       = esc_html__( 'The installation of %1$s failed.', 'zap' );

							if ( $this->tgmpa->is_automatic ) {
								// Automatic activation strings.
								$this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'zap' );
								$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'zap' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'zap' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'zap' ) . '</span>.</a>';
								$this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations and activations have been completed.', 'zap' );
								$this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'zap' );
							} else {
								// Default installation strings.
								$this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'zap' );
								...
								$this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations have been completed.', 'zap' );
								$this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing Plugin %1$s (%2$d/%3$d)', 'zap' );
							}
						}
					}
