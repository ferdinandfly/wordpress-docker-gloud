...
class ZO_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            /* search */
            elseif (is_search()):
                printf( __( 'Search Results for: %s', 'zap' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                esc_html_e( '404', 'zap');
            /* other */
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( __( 'Author: %s', 'zap' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( __( 'Day: %s', 'zap' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( __( 'Month: %s', 'zap' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'zap' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( __( 'Year: %s', 'zap' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'zap' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                esc_html_e( 'Asides', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                esc_html_e( 'Galleries', 'zap');
            elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
                esc_html_e( 'Images', 'zap');
            elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
                esc_html_e( 'Videos', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
                esc_html_e( 'Quotes', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
                esc_html_e( 'Links', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
                esc_html_e( 'Statuses', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
                esc_html_e( 'Audios', 'zap' );
            elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
                esc_html_e( 'Chats', 'zap' );
            /* woocommerce */
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.esc_html__("Search", 'zap').'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.esc_html__("404 - Page not Found", 'zap').'</li>';
        }
        /* archive */
